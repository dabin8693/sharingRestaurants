...
interface ItemDao {
    ...
    @Query("SELECT * FROM item WHERE (item.title LIKE :query OR item.body LIKE :query)")
    fun searchByTitleOrBody(query: String?): LiveData<List<ItemEntity>>//제목 또는 본문에 해당 문자열이 있으면 검색
